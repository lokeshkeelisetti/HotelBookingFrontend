	...
	render() {
		const HomeWithDetails = () => {
			return (
				<Home
					...
					hotelAdmins={this.state.hotelAdmins}
		...
		const hotelWithId = ({ match }) => {
			return (
				<Hotel
					hotel={
						this.state.hotels.filter(
							(hotel) => hotel.id === parseInt(match.params.hotelId, 10)
						)[0]
					}
				/>
			);
		...
		const SearchHotels = () => {
			return (
				<Search
					hotels={this.state.searchHotelResults}
					handleSearchHotel={this.handleSearchHotel}
				/>
			);
		};
		...
		const RenderPreviousBookings = () => {
			return <PreviousBookings bookings={this.state.previousBookings} />;
		};

		const RenderUpcomingBookings = () => {
			return <UpcomingBooking bookings={this.state.upcomingBookings} />;
		};
					...
					<TransitionGroup className="mb-auto flex-grow-1">
						<Switch>
							...
							<Route
								...
								component={RenderPreviousBookings}
							/>
							<Route
								exact
